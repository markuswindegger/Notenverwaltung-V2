...
class Noten extends DBObject
{
  ...
  {
    $con = AppDBO::getInstance();
    $updatequery = "update Noten set deleteTime = :time, deleteUser = :user, valid = 0 where N_Id = :ident";
  ...
  {
    $con = AppDBO::getInstance();
    $insertquery = "insert into Noten (note, F_Id, S_Id, createUser, 
			createTime, valid) values (:note, :fach, :schueler, :user, :time, 1)";
      
    if(!($stm = $con->prepare($insertquery)))
      {
	return MyError::raiseError('prepare_error', SQL_ERROR, $con->getErrorMsg());
      }
    $stm->bindValue(':note', $this->note);
  ...
  public function validate()
  {
    ...
    if($this->note == NULL || trim($this->note) === "")
      {
	return MyError::raiseError(gettext('set_note'), SYS_ERROR);
      }
